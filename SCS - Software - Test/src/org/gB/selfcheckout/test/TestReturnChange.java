...
public class TestReturnChange{
    ...
    public void testBanknoteOutput1() throws OverloadException, EmptyException, DisabledException{
        ...
        //the third time we try to remove dangling banknote, we don't have any more banknotes in the dispenser
        //however, the empty exception is caught inside the banknoteRemoved, so if we keep simulating it we will get a simulation exception
        //right now we just print the empty exception inside banknoteRemoved, but we can do other things there in the later 
        //but it will actually be impossible to remove a dangling banknote that wasn't there in the first place ... so we will not test it here
    ...
    public void testBanknoteOutput2() throws OverloadException, EmptyException, DisabledException{
        initBanknote1();
        BigDecimal expectSum= new BigDecimal(b1.getValue() + b2.getValue()*2+b0.getValue()*3+b3.getValue()*2);
    ...
    public void testCoinOutput1() throws OverloadException, EmptyException, DisabledException{
        ...
        //NOTE: we need to remove the coins in the tray before return the change, or else 
        //the hardware/JUnit non-deterministically have coins in it, even though NOTHING has outputted
        ///*
        state.scs.coinTray.collectCoins();
        //*/
