...
public class ReturnChange implements BanknoteSlotObserver{
    ...
    //this array keeps track of how many remaining banknotes we need to give for each change
    //it is used for the banknote slot observer, where if we emit a banknote, we need to wait for the user to take away the dangling banknote.
    private int [] banknoteChangeList;
    //the current index for the banknote we are trying to return as change to the user
    //NOTE: we maintain an invariant for curBanknoteIndex, where everything to the left of curBanknoteIndex are paid to the user (i.e. for all 0 <= i <= curBanknoteIndex, banknoteChangeList[i]=0)
    private int curBanknoteIndex;

    public ReturnChange(State state){
        this.state=state;
        this.bankDenom = state.scs.banknoteDenominations;
        //to convert the list to a square bracket BigDecimal[], according to the standard library documentation, if the array does not fit in the inputted array, it will allocate a new buffer and return that
        //also discussed in https://stackoverflow.com/questions/9572795/convert-list-to-array-in-java
    ...
    /**
     * Does a divides b with repeated subtracting
     * because if we use the big decimal division, we need to specify a rounding, which produces non-deterministic results with floating point arithmetic
	 * @param a 
	 * @param b
     *      
     */
    ...
    /**
    * Pure function that takes in the ammount to return, the bank and coin denominations it will return the ammonut of change floored to the smallest coin denomination
    * It will return a pair of lists that corresponds to each denomination.
    * First projection of the pair corresponds to the banknotes, and the second projection corresponds to the coins
    * Precondition: ammOfChange is non-negative, bankDenom and coinDenom are sorted in ascending order
	 * @param ammOfChange
     *      The ammount of change to be returned
	 * @param bankDenom
     *      The bank note denominations
	 * @param coinDenom
    ...
    */
    private Pair<int [], int []> getChangeDenomCounts(BigDecimal ammOfChange, int [] bankDenom, BigDecimal [] coinDenom){
        ...
        //iterate over the banknote denominations backwards, since in the Main.java, we know the it is sorted in ascending order, so we just iterate backwards
        //since ammLeft is non-negative, when we divide, the result should be non-negative
		for(int i = bankDenom.length-1; i >= 0; i--){
            //if we are essentially 0, we are done
            if (checkSmallerThanEpsilon(ammLeft)) break;
    ...
    /**
        * Returns the coins specified in the input coinChangeList
        * Precondition: elements in coinChangeList are non-negative
	    * @param coinChangeList
        *       The number of coins to return that corresponds to each index of the coin denomination in the member variable coinDenom
	    * @throws OverloadException
	    * @throws EmptyException
	    * @throws DisabledException
        *   These exceptions are propagated up from the coin dispenser emit() function
    */
    ...
    /**
       * Returns a banknote, if we still have remaining banknotes to return as change. Otherwise, this function does nothing
       * The state of curBanknoteIndex is assumed to be in the invariant, described in the declaration of curBanknoteIndex
	    * @throws OverloadException
	    * @throws EmptyException
	    * @throws DisabledException
        *   These exceptions are propagated up from the banknote dispenser emit() function
    */
    private void returnABanknote() throws EmptyException, DisabledException, OverloadException{
        //if the current banknote change is used up (it is 0), go to the next index with non-zero banknote change
        //note if everything in banknoteChangeList is 0, we will have curBanknoteIndex=banknoteChangeList.length at the end
        while (curBanknoteIndex<banknoteChangeList.length && banknoteChangeList[curBanknoteIndex] == 0) ++curBanknoteIndex;
        
        //if there are nothing to return for the banknote, we just do nothing
        if (curBanknoteIndex==banknoteChangeList.length) return;
    ...
    /**
	    ...
	    * @throws EmptyException
	    * @throws DisabledException
        *   These exceptions are propagated up from returnCoins() and returnABanknote()
    */
