...
public class AddItemToBag implements ElectronicScaleObserver {
	 ...
	 * 
	 * @param state
	 * 		The instance of the system state which contains the scale that will
	 *		interact with items.
	 */
	...
	/**
	 * Handles weight change updates from the bagging area scale (if the scale
	 * is enabled). If the waitingForBagging flag is set in the state and the
	 * weight is as expected, the flag will be un-set and the system's barcode
	 * scanners are enabled. If the flag is not set, then an error will be
	 * issued to the interface if the weight is not as expected.
	 *
	 * @param scale
	 * 		The scale experiencing a weight change.
	 * @param weightInGrams
	 * 		The weight currently on the indicated scale.
	 */
	@Override
	public void weightChanged(ElectronicScale scale, double weightInGrams) {
		...
		if (state.waitingForBagging) { // If an item is expected,
			...
			System.out.println("expected: "+state.getExpectedWeight()+" actual: "+weightInGrams);
	 ...
	 * 
	 * @param scale
	 * 		The scale experiencing an overload.
	 */
	@Override
	...
	/**
	 * Notifies the interface that a scale is no longer in an overload
	 * condition (if the scale is enabled).
	 * 
	 * @param scale
	 * 		The scale no longer experiencing an overload.
	 */
	@Override
